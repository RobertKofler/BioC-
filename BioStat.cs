...
The Initial Developer of the Original Code is some guy whose name I unfortunatelly forgot; If you
are the developer and reading this I sincerly appologise and ask you to please contact me at robert@kofler.or.at
so that I can enter you as the initial developer.
Carrer del Baluard; 73 2°1a
08003 Barcelona
All Rights Reserved.

Contributor(s): 
unknown initial developer,
Dr. Robert Kofler.
...
namespace Bio.Stat
{
        ...
        {
            private List<double> basis;
            private List<double> toTest;
            private int? positionOfToTest;
            private double? p;
            private class MannWhitneyRank
            {
                public double variable;
                public byte group;
                public double rank;
            }
            private class MannWhitneyRankSort : IComparer<MannWhitneyRank>
            {
                public MannWhitneyRankSort()
                {
                }
                int IComparer<MannWhitneyRank>.Compare(MannWhitneyRank x, MannWhitneyRank y)
                {
                    if (x.variable < y.variable) return -1;
                    else if (x.variable > y.variable) return 1;
                    else return 0;
                }
            }
            ...
            public MannWhitneyU(List<double> basis, List<double> toTest)
            {
                this.basis = basis;
                this.toTest = toTest;
            }
            ...
            /// <summary>
            /// Determines the mean position of the toTest collection
            /// returns -1 if on the left hand side of basis; 1 if on the right hand side of basis, and 0 if equal.
            /// </summary>
            public int PositionOfToTest
            {
                get
                {
                    if (positionOfToTest == null)
                        this.Test();
                    return positionOfToTest.Value;
                }
            }


            public double P_TwoSided
            {
                get
                {
                    ...
                    if (this.p == null)
                        this.Test();
                    return 2.0 * this.p.Value;
                }
            }

            public double P_OneSided
            {
                get
                {
                    if (this.p == null)
            ...
            /// <summary>
            /// Uses the normal approximatin to calculate the significance value,
            /// this is appropriate for large sample sizes.
            /// For small sample sizes the tables should be used
            /// </summary>
            private void Test()
            {
                List<MannWhitneyRank> ranks = new List<MannWhitneyRank>(this.basis.Count + this.toTest.Count);
                foreach (double d in basis)
                {
                    MannWhitneyRank r = new MannWhitneyRank();
                    r.variable = d;
                    r.group = (byte)'b';
                    ranks.Add(r);
                }
                foreach (double d in toTest)
                {
                    MannWhitneyRank r = new MannWhitneyRank();
                    r.variable = d;
                    r.group = (byte)'t';
                ...
                ranks.Sort(new MannWhitneyRankSort());

                //Assign Ranks
                for (int i = 0; i < ranks.Count; i++)
                {
                    int properRank = i + 1;
                    double aggregateRank = (double)properRank;
                    int countWithSameValue = 1;
                    bool common = false;
                    while ((i < ranks.Count - 1) && (ranks[i].variable == ranks[i + 1].variable))
                    {
                        aggregateRank += (properRank + countWithSameValue);
                        countWithSameValue++;
                        i++;
                        common = true;
                    }
                    ...
                    if (common)
                    {
                        assignRank = aggregateRank / countWithSameValue;

                        int start = i;
                        for (; i < start + countWithSameValue; i++)
                        {


                            ranks[i].rank = assignRank;
                        }
                        i--;
                    }
                    else
                    {
                        ranks[i].rank = (double)properRank;
                    }
                }


                //Calculate rank sums
                double rankSumBasis = 0.0;
                double rankSumToTest = 0.0;
                foreach (MannWhitneyRank r in ranks)
                {
                    if (r.group == (byte)'b') rankSumBasis += r.rank;
                    else if (r.group == (byte)'t') rankSumToTest += r.rank;
                    else throw new ArgumentException();
                }

                //Controll
                double n = ((double)basis.Count) + ((double)toTest.Count);
#if DEBUG
                double sumofallRanks = n * (n + 1.0) / 2.0;
                double actualSummOfAllRanks = rankSumBasis + rankSumToTest;
#endif
                if (rankSumBasis + rankSumToTest != n * (n + 1.0) / 2.0) throw new InvalidOperationException();


                //Umrechnen der rangsummen
                double nBasis = ((double)basis.Count);
                double nToTest = ((double)toTest.Count);
                double UBasis = rankSumBasis - ((nBasis * (nBasis + 1.0)) / 2.0);
                double UToTest = rankSumToTest - ((nToTest * (nToTest + 1.0) / 2.0));

                //Controll
                if (UBasis + UToTest != nBasis * nToTest) throw new InvalidOperationException();
                ...
                {
                    u = UBasis;
                    this.positionOfToTest = 1;
                }
                else if (UBasis > UToTest)
                {
                    u = UToTest;
                    this.positionOfToTest = -1;
                }
                ...
                {
                    u = UBasis;
                    this.positionOfToTest = 0;
                }

                double testUpper = ((nBasis * nToTest) / 2.0) - u;
                double testLower = Math.Sqrt((nBasis * nToTest * (nBasis + nToTest + 1.0)) / 12.0);
                double test = testUpper / testLower;


                //ToDo check with one and two directions
                this.p = 1.0 - (1.0 * Stat.Normal(test));
        ...
        public class PoissonDistribution
        {
            ...
            /// <param name="k"></param>
            /// <returns></returns>
            public double CumulativeProbability_Right(int k)
            {
                if (k < 0 || lambda < 0) return 0.0;

                return Stat.igam((double)(k + 1), lambda);
            }
            ...
            /// <returns></returns>
            public double CumulativeProbability_Left(int k)
            {
                if (k < 0 || lambda < 0) return 0.0;

                return Stat.igamc((double)(k + 1), lambda);
            }


            /// <summary>
            /// Returns the exact probability for the event k
            /// </summary>
            public double Probability(int k)
            {
                return 1 - (CumulativeProbability_Right(k) + CumulativeProbability_Left(k - 1));
            }
        }
        ...
        public class Stat
        {
            ...
            /// <param name="x">double value</param>
            /// <returns></returns>
            public static double Poissonc(int k, double lambda)
            {
                if (k < 0 || lambda < 0) return 0.0;

                return igam((double)(k + 1), lambda);
            }
        ...
        public class RandomProvider
        {
            ...
            /// <summary>
            /// Create a new instance of RandomProvider,
            /// the seed is automatically set as Environment.Ticks
            /// </summary>
        ...
        /// (Random distribution of the numbers confirmed with SPSS 15.0)
        /// </summary>
        public class RandomGaussian
        {
            ...
            /// <summary>
            /// Create a new instance of a Gaussian random number generater,
            /// the seed is automatically set (Environment.Ticks)
            /// </summary>
            ...
            /// <summary>
            /// Returns a new Gaussian random number with zero mean and unit (1)
            /// variance. random number are element of real numbers (positive + negative); SPSS confirmed
            /// 
            /// </summary>
            public double NextGaussian()
            {
                ...
                else
                {
                    ...
                    while (rsq >= 1.0 || rsq == 0.0)
                    {
                        v1 = 2.0 * m_RNG1.NextDouble() - 1.0;
                        v2 = 2.0 * m_RNG1.NextDouble() - 1.0;
                        rsq = v1 * v1 + v2 * v2;
                    }
                    fac = System.Math.Sqrt(-2.0
                       * System.Math.Log(rsq, System.Math.E) / rsq);
            ...
            /// <summary>
            /// Provides a new Gaussian random number, around the specified mean value
            /// and the given standard deviation; SPSS confirmed
            /// </summary>
